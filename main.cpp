...
QString** playerTurn(QString** board) {
    while (true) {
        std::cout << std::endl << "Type in a number that is on the board to place an X there >";

        int pos;
        std::string posResponse;
        std::cin >> posResponse;
        pos = std::stoi(posResponse);

        // Make sure we can play there too
        if (pos == 1 && board[0][0] == "1") {
            board[0][0] = "X";
            break;
        }
        if (pos == 2 && board[0][1] == "2") {
            board[0][1] = "X";
            break;
        }
        if (pos == 3 && board[0][2] == "3") {
            board[0][2] = "X";
            break;
        }
        if (pos == 4 && board[1][0] == "4") {
            board[1][0] = "X";
            break;
        }
        if (pos == 5 && board[1][1] == "5") {
            board[1][1] = "X";
            break;
        }
        if (pos == 6 && board[1][2] == "6") {
            board[1][2] = "X";
            break;
        }
        if (pos == 7 && board[2][0] == "7") {
            board[2][0] = "X";
            break;
        }
        if (pos == 8 && board[2][1] == "8") {
            board[2][1] = "X";
            break;
        }
        if (pos == 9 && board[2][2] == "9") {
            board[2][2] = "X";
            break;
        }
    }
...
int main() {
    srand(time(0));
    std::cout << "Welcome to Tic-Tac-Toe by CyanCoding!" << std::endl;
    std::cout << "You play as 'X' and the computer plays as 'O'." << std::endl << std::endl;
    std::cout << "Type in 'Play' to start a game against the computer or 'PvP' to start a game with two players > ";

    QString response;
    std::string stdResponse;
    std::cin >> stdResponse;

    response = QString::fromStdString(stdResponse).toLower();

    // Generate the board array
    QString** board = new QString*[3];
    for (int i = 0; i < 3; i++) {
        board[i] = new QString[3];
    }

    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            board[i][j] = QString::number(i + j);
        }
    }

    // Handle the "board" response
    if (response == "board") {
        drawBoard(board);
    }
    ...
    std::cout << "Type in a difficulty level (Easy, Normal, Impossible) > ";
    QString difficulty;
    std::string difficultyResponse;
    std::cin >> difficultyResponse;

    difficulty = QString::fromStdString(difficultyResponse).toLower();

    if (difficulty == "impossible") {
        std::cout << std::endl << "Deciding who will go first..." << std::endl;
        int rp = (rand() % 2);
        if (rp == 0) {
            // if the player is selected to go first
            printBlank();
            std::cout << "You have been selected to play first! Good luck!" << std::endl;
            drawBoard(board);
            ...
            // If the center is already filled, pick a corner
            // we don't need to check if it's filled because
            // the user already played in the center
            else {
                int random_play = (rand() % 4);
